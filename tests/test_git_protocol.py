...
from src.infrastructure.git_protocol import (
    PktLineParser,
    GitProtocolValidator,
    GitHeaders,
    GitService,
    GitContentType
)
...
class TestGitProtocolValidator:
    ...
    def test_validate_service_valid(self):
        ...
        assert GitProtocolValidator.validate_service("git-upload-pack") == GitService.UPLOAD_PACK
        assert GitProtocolValidator.validate_service("git-receive-pack") == GitService.RECEIVE_PACK
    ...
    def test_validate_ref_name_valid(self):
        ...
        assert GitProtocolValidator.validate_ref_name("refs/remotes/origin/main") is True
    ...
    def test_validate_object_id_valid(self):
        ...
        assert GitProtocolValidator.validate_object_id("0123456789abcdef" * 2 + "01234567") is True
    ...
    def test_validate_object_id_invalid(self):
        ...
        assert GitProtocolValidator.validate_object_id("a" * 41) is False  # Wrong length
        assert GitProtocolValidator.validate_object_id("g" * 40) is False  # Invalid chars
...
class TestGitHeaders:
    ...
    def test_validate_content_type_valid(self):
        ...
        assert GitHeaders.validate_content_type(
    ...
    def test_extract_service_valid(self):
        ...
        assert GitHeaders.extract_service("service=git-upload-pack") == GitService.UPLOAD_PACK
        assert GitHeaders.extract_service("service=git-receive-pack") == GitService.RECEIVE_PACK
        assert GitHeaders.extract_service("foo=bar&service=git-upload-pack&baz=qux") == GitService.UPLOAD_PACK
...
class TestGitContentType:
    ...
    def test_for_service_advertisement(self):
        ...
        assert GitContentType.for_service_advertisement(GitService.UPLOAD_PACK) == \
            "application/x-git-upload-pack-advertisement"
        assert GitContentType.for_service_advertisement(GitService.RECEIVE_PACK) == \
            "application/x-git-receive-pack-advertisement"
    
    def test_for_service_request(self):
        """Test getting content type for service request"""
        assert GitContentType.for_service_request(GitService.UPLOAD_PACK) == \
            "application/x-git-upload-pack-request"
        assert GitContentType.for_service_request(GitService.RECEIVE_PACK) == \
            "application/x-git-receive-pack-request"
    
    def test_for_service_result(self):
        """Test getting content type for service result"""
        assert GitContentType.for_service_result(GitService.UPLOAD_PACK) == \
            "application/x-git-upload-pack-result"
        assert GitContentType.for_service_result(GitService.RECEIVE_PACK) == \
