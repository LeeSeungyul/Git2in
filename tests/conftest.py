...
def test_user() -> User:
    """Create a test user"""
    return User(username="testuser", email="test@example.com", is_active=True)
...
async def initialized_repo(
    ...
    repo_path = await repository_service.create_repository(
        test_namespace, test_repository
    )
    ...
    subprocess.run(
        ["git", "init"], cwd=str(repo_path.parent), check=True, capture_output=True
    )

    subprocess.run(
        ["git", "add", "."], cwd=str(repo_path.parent), check=True, capture_output=True
    )
    ...
    bare_repo_path = filesystem_manager.get_repository_path(
        test_namespace.name, f"{test_repository.name}.git"
    )
...
def mock_git_http_backend(monkeypatch):
    ...
    monkeypatch.setattr("src.api.git_http.GitBackendProcess", MockGitBackend)
...
def test_repository_path(test_base_dir: Path) -> Path:
    ...
    subprocess.run(
        ["git", "init", "--bare"], cwd=str(repo_path), check=True, capture_output=True
    )
