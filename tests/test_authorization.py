...
from src.core.authorization.models import (
    ...
    PermissionGrant, AuthorizationRequest, AuthorizationResult,
    RolePermissions, PermissionInheritance
)
from src.core.authorization.service import AuthorizationService, authorization_service
from src.core.auth.models import TokenClaims, TokenScope, TokenType
from src.infrastructure.audit.logger import AuditLogger, AuditAction, AuditResult
from src.infrastructure.security.ip_filter import (
    IPFilterService, IPFilterRule, IPFilterAction, IPFilterConfig
)
...
class TestPermissionMatrix:
    ...
    def test_add_and_remove_grant(self):
        ...
        grant = PermissionGrant(
            ...
            resource_type=ResourceType.REPOSITORY,
            resource_id="namespace/repo",
            permissions=[Permission(resource_type=ResourceType.REPOSITORY, action=Action.READ)],
            granted_by="admin"
        )
...
class TestAuthorizationService:
    ...
    def test_parse_resource_path(self):
        ...
        # Git endpoint
        resource_type, resource_id = service.parse_resource_path("/git/namespace/repo.git/info/refs")
        assert resource_type == ResourceType.REPOSITORY
        assert resource_id == "namespace/repo"
        
        # Namespace endpoint
        resource_type, resource_id = service.parse_resource_path("/namespaces/my-namespace/repos")
        assert resource_type == ResourceType.NAMESPACE
        assert resource_id == "my-namespace"
        
        # User endpoint
        resource_type, resource_id = service.parse_resource_path("/users/user123/profile")
...
class TestIPFiltering:
    ...
    def test_resource_specific_rules(self):
        ...
        allowed, reason = service.check_ip(
            "172.16.1.1",
            resource_id="repo:namespace/secure-repo"
        )
        assert allowed
