...
class TestAuthenticationAPI:
    ...
    async def test_create_token_invalid_credentials(self, async_client: AsyncClient):
        ...
        response = await async_client.post(
            "/api/v1/auth/token",
            json={"username": "wrong", "password": "wrong", "scopes": []},
        )
    ...
    async def test_refresh_token(self, async_client: AsyncClient):
        ...
        response = await async_client.post(
            "/api/v1/auth/token",
            json={"username": "admin", "password": "admin", "scopes": ["repo:read"]},
        )
    ...
    async def test_introspect_token(self, async_client: AsyncClient):
        ...
        response = await async_client.post(
            "/api/v1/auth/introspect", json={"token": token}
        )
    ...
    async def test_introspect_invalid_token(self, async_client: AsyncClient):
        ...
        response = await async_client.post(
            "/api/v1/auth/introspect", json={"token": "invalid.token.here"}
        )
    ...
    async def test_revoke_token(self, async_client: AsyncClient):
        ...
        access_token = token_service.create_access_token(
            user_id=user_id, username="testuser", scopes=[TokenScope.TOKEN_REVOKE]
        )
        ...
        response = await async_client.post(
            "/api/v1/auth/revoke",
            json={"token_id": claims.jti, "reason": "Test revocation"},
    ...
    async def test_list_sessions(self, async_client: AsyncClient):
        ...
        user_id = uuid4()
        access_token = token_service.create_access_token(
            user_id=user_id, username="testuser", scopes=[TokenScope.USER_READ]
        )
    ...
    async def test_terminate_all_sessions(self, async_client: AsyncClient):
        ...
        for i in range(3):
            token = token_service.create_access_token(
                user_id=user_id, username="testuser", scopes=[TokenScope.USER_WRITE]
            )
        ...
        response = await async_client.delete(
            "/api/v1/auth/sessions", headers={"Authorization": f"Bearer {tokens[0]}"}
        )
    ...
    async def test_create_api_key(self, async_client: AsyncClient):
        ...
        admin_token = token_service.create_access_token(
            user_id=uuid4(), username="admin", scopes=[TokenScope.TOKEN_CREATE]
        )

        response = await async_client.post(
            "/api/v1/auth/api-key",
            json={"name": "Test API Key", "scopes": ["repo:read"], "ttl_days": 30},
...
class TestAuthenticationMiddleware:
    ...
    async def test_protected_endpoint_with_valid_token(self, async_client: AsyncClient):
        ...
        with patch("src.api.git_http.FilesystemManager") as mock_fs:
            mock_fs.return_value.get_repository_path.return_value.exists.return_value = (
                True
            )
    ...
    async def test_revoked_token(self, async_client: AsyncClient):
        ...
        # Create token
        token = token_service.create_access_token(
            user_id=uuid4(), username="testuser", scopes=[TokenScope.USER_READ]
        )
        ...
        await revocation_manager.revoke_token(
            jti=claims.jti, token_exp=claims.exp, reason="Test"
        )
