...
from src.infrastructure.hooks import (AccessControlHook, Hook, HookContext,
                                      HookEvent, HookManager, LoggingHook,
                                      StatisticsHook, WebhookHook)
...
class TestHookContext:
    ...
    def test_hook_context_add_result(self):
        ...
        context = HookContext(event=HookEvent.POST_RECEIVE, repository=Mock())
    ...
    def test_hook_context_add_error(self):
        ...
        context = HookContext(event=HookEvent.PRE_RECEIVE, repository=Mock())
...
class TestStatisticsHook:
    ...
    async def test_statistics_hook_execute(self):
        ...
        context1 = HookContext(event=HookEvent.POST_RECEIVE, repository=repo)
        ...
        context2 = HookContext(event=HookEvent.POST_RECEIVE, repository=repo)
    ...
    async def test_statistics_hook_multiple_events(self):
        ...
        # Check last context
        assert (
            contexts[-1].results["statistics"]["event_count"] == 2
        )  # repo1:post-receive
...
class TestAccessControlHook:
    ...
    async def test_access_control_hook_allow(self):
        ...
        context = HookContext(event=HookEvent.PRE_PUSH, repository=Mock())
    ...
    async def test_access_control_hook_deny(self):
        ...
        context = HookContext(event=HookEvent.PRE_PUSH, repository=repo)
...
class TestWebhookHook:
    ...
    async def test_webhook_hook_success(self):
        ...
        context = HookContext(event=HookEvent.POST_PUSH, repository=repo)
    ...
    async def test_webhook_hook_failure(self):
        ...
        context = HookContext(event=HookEvent.POST_PUSH, repository=Mock())
...
class TestHookManager:
    ...
    async def test_execute_hooks(self):
        ...
        result_context = await manager.execute_hooks(HookEvent.PRE_PUSH, context)
    ...
    async def test_execute_hooks_stop_on_error(self):
        ...
        result_context = await manager.execute_hooks(
            HookEvent.PRE_PUSH, context, stop_on_error=True
        )
    ...
    async def test_execute_hooks_timeout(self):
        ...
        result_context = await manager.execute_hooks(
            HookEvent.PRE_PUSH, context, timeout=0.1
        )
