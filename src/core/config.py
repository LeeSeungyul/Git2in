...
class Settings(BaseSettings):
    ...
    repository_base_path: Path = Field(
        ...
        description="Base path for Git repositories"
    )
    ...
    api_key_ttl: int = Field(
        default=31536000,
        description="API key TTL in seconds (default: 1 year)"
    )
    key_rotation_days: int = Field(
        default=30,
        description="Days between signing key rotations"
    )
    key_overlap_days: int = Field(
        default=7,
        description="Days to keep old keys for verification"
    )
    ...
    git_http_backend_path: str = Field(
        ...
        description="Path to git-http-backend binary"
    )
    ...
    metrics_token: Optional[str] = Field(
        default=None,
        description="Optional token for metrics access"
    )
