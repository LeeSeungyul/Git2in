...
class NamespaceService:
    ...
    async def get_namespace(
        self, namespace_id: UUID, user_id: Optional[UUID] = None
    ) -> Optional[Namespace]:
        """Get namespace by ID"""
    ...
    async def delete_namespace(self, namespace_id: UUID, cascade: bool = False) -> bool:
        """Delete namespace"""
        namespace = self._namespaces.get(namespace_id)
        if not namespace:
            return False
        ...
        if filters:
            if filters.search:
                ...
                namespaces = [
                    ...
                    or (ns.description and search_lower in ns.description.lower())
                ]

            if filters.visibility:
                namespaces = [
                    ns for ns in namespaces if ns.visibility == filters.visibility
                ]

            if filters.owner_id:
                namespaces = [
                    ns for ns in namespaces if ns.owner_id == filters.owner_id
                ]
    ...
    async def add_member(
        self, namespace_id: UUID, user_id: UUID, role: str, added_by: UUID
    ) -> bool:
    ...
    async def remove_member(self, namespace_id: UUID, user_id: UUID) -> bool:
