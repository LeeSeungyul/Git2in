...
from src.core.auth.models import (Token, TokenClaims, TokenHeader,
                                  TokenRequest, TokenScope, TokenType,
...
class TokenService:
    ...
    def generate_token(self, request: TokenRequest) -> str:
        ...
        token_string = self.signer.sign_token(header.to_dict(), claims.to_dict())
    ...
    def validate_token(self, token_string: str) -> TokenValidationResult:
        ...
        if not header or not payload:
            ...
            return TokenValidationResult.failure(
                "Failed to decode token", "DECODE_ERROR"
            )
        ...
        try:
            ...
            if claims.is_expired():
                logger.warning(
                    "token_validation_failed", reason="expired", token_id=claims.jti
                )
                return TokenValidationResult.failure(
            ...
            logger.debug("token_validated", token_id=claims.jti, user_id=claims.sub)
    ...
    def create_access_token(
        self, user_id: UUID, username: str, scopes: List[TokenScope], **kwargs
    ) -> str:
    ...
    def create_refresh_token(self, user_id: UUID, username: str, **kwargs) -> str:
    ...
    def refresh_access_token(self, refresh_token_string: str) -> Optional[str]:
        ...
        if result.claims.token_type != TokenType.REFRESH:
            logger.warning(
                "refresh_token_wrong_type", token_type=result.claims.token_type.value
            )
            return None
