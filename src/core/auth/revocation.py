...
class RevocationStore:
    ...
    async def add(
        self, jti: str, expires_at: datetime, reason: Optional[str] = None
    ) -> None:
...
class InMemoryRevocationStore(RevocationStore):
    ...
    async def add(
        ...
        with self._lock:
            revocation = TokenRevocation(
                jti=jti, expires_at=expires_at, reason=reason, revoked_by=revoked_by
            )
            self._revocations[jti] = revocation

            logger.info("token_revoked", jti=jti, reason=reason, revoked_by=revoked_by)
    ...
    async def cleanup_expired(self) -> int:
        ...
        with self._lock:
            expired_jtis = [
                jti for jti, rev in self._revocations.items() if rev.is_expired()
            ]
            ...
            if expired_jtis:
                logger.info("expired_revocations_cleaned", count=len(expired_jtis))
    ...
    async def list_all(self) -> List[TokenRevocation]:
        ...
        with self._lock:
            ...
            active_revocations = [
                rev for rev in self._revocations.values() if not rev.is_expired()
            ]
...
class RevocationManager:
    ...
    async def revoke_tokens_by_user(
        self, user_id: str, token_ids: List[str], reason: str = "User tokens revoked"
    ) -> int:
        ...
        logger.info("user_tokens_revoked", user_id=user_id, count=count, reason=reason)
    ...
    async def get_statistics(self) -> Dict:
        ...
        stats = {
            ...
            "revoked_last_hour": sum(
                1 for r in revocations if (now - r.revoked_at).total_seconds() < 3600
            ),
            "revoked_last_day": sum(
                1 for r in revocations if (now - r.revoked_at).total_seconds() < 86400
            ),
            "by_reason": {},
        }
