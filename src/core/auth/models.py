...
class TokenClaims(BaseModel):
    ...
    iat: int = Field(
        default_factory=lambda: int(time.time()), description="Issued at timestamp"
    )
    exp: int = Field(..., description="Expiration timestamp")
    jti: str = Field(
        default_factory=lambda: str(uuid4()), description="JWT ID for uniqueness"
    )
    ...
    ip_address: Optional[str] = Field(
        None, description="IP address token was issued to"
    )
        ...
        if v and info.data.get("scopes"):
            ...
            if namespace_scopes and not v:
                raise ValidationError(
                    "Namespace required for namespace-scoped permissions"
                )
        return v
    ...
    def validate_repository(cls, v: Optional[str], info) -> Optional[str]:
        ...
        if v and info.data.get("scopes"):
            ...
            if repo_scopes and not v:
                raise ValidationError(
                    "Repository required for repository-scoped permissions"
                )
        return v
...
class Token(BaseModel):
    ...
    def encode_payload(self) -> str:
        ...
        header_b64 = (
            base64.urlsafe_b64encode(header_json.encode()).rstrip(b"=").decode()
        )

        # Encode claims
        claims_json = json.dumps(self.claims.to_dict(), separators=(",", ":"))
        claims_b64 = (
            base64.urlsafe_b64encode(claims_json.encode()).rstrip(b"=").decode()
        )
...
class TokenRevocation(BaseModel):
    ...
    expires_at: datetime = Field(
        ..., description="When the revocation record can be deleted"
    )
...
class TokenValidationResult(BaseModel):
    ...
    @classmethod
    def failure(
        cls, error: str, error_code: str = "INVALID_TOKEN"
