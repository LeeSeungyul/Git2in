...
class Namespace(BaseModel):
    ...
    created_at: datetime = Field(
        ...
        description="Timestamp of namespace creation"
    )
    ...
    settings: Dict[str, Any] = Field(
        ...
        description="Namespace-level configuration settings"
    )
    ...
    RESERVED_NAMES: ClassVar[Set[str]] = {
        "api", "admin", "root", "system", "git", "http", "https",
        "ssh", "login", "logout", "register", "settings", "profile",
        "dashboard", "metrics", "health", "status", "about", "help",
        "docs", "documentation", "public", "private", "test", "tests"
    }
    ...
    def validate_settings(cls, v: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "max_repo_size_mb" in v:
            if not isinstance(v["max_repo_size_mb"], (int, float)) or v["max_repo_size_mb"] <= 0:
