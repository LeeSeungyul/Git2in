...
class Repository(BaseModel):
    ...
    owner_id: UUID = Field(
        ...,
        description="UUID of the repository owner"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow,
        description="Timestamp of repository creation"
    )
    ...
    NAME_PATTERN: ClassVar[re.Pattern] = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]$")
    
    # Reserved repository names
    RESERVED_NAMES: ClassVar[Set[str]] = {
        ".git", ".gitignore", "HEAD", "config", "description",
        "hooks", "info", "objects", "refs", "branches", "tags"
    }
    ...
    def validate_size(cls, v: int) -> int:
        max_size = getattr(settings, "max_repo_size_bytes", 1024 * 1024 * 1024)  # Default 1GB
        if v > max_size:
            raise ValueError(f"Repository size exceeds maximum allowed size of {max_size} bytes")
        return v
