...
class Token(BaseModel):
    ...
    user_id: UUID = Field(..., description="UUID of the token owner")
    name: Optional[str] = Field(
        None, max_length=128, description="Optional token name for identification"
    )
    token_hash: str = Field(..., description="SHA256 hash of the actual token")
    scopes: List[str] = Field(
        default_factory=list, description="List of token scopes/permissions"
    )
    ...
    is_revoked: bool = Field(
        default=False, description="Whether the token has been revoked"
    )
    revoked_at: Optional[datetime] = Field(
        None, description="Timestamp when token was revoked"
    )
    last_used: Optional[datetime] = Field(
        None, description="Last time the token was used"
    )
    usage_count: int = Field(
        default=0, ge=0, description="Number of times token has been used"
    )
