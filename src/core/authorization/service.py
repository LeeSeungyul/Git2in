...
class AuthorizationService:
    ...
    def check_permission(
        ...
        cache_key = self._make_cache_key(
            token_claims.sub, resource_type, action, resource_id
        )
        ...
        result = self._perform_authorization(
            token_claims, resource_type, action, resource_id
        )
    ...
    def _perform_authorization(
        ...
        token_permissions = self._parse_token_scopes(
            token_claims.scopes, resource_type, resource_id
        )
        ...
        for perm in token_permissions:
            if perm.matches(resource_type, action, resource_id):
                return AuthorizationResult.allow(
                    reason="Permission granted by token scope", permissions=[perm]
                )

        # Check matrix permissions for user
        if token_claims.user_id:
            user_permissions = self.permission_matrix.get_user_permissions(
                str(token_claims.user_id), resource_type, resource_id
            )

            for perm in user_permissions:
                if perm.matches(resource_type, action, resource_id):
                    return AuthorizationResult.allow(
                        reason="Permission granted by user role", permissions=[perm]
                    )
        ...
        if inherited_perms:
            return AuthorizationResult.allow(
                reason="Permission granted by inheritance", permissions=inherited_perms
            )

        # Check namespace/repository restrictions in token
        if not self._check_resource_restrictions(
            token_claims, resource_type, resource_id
        ):
            return AuthorizationResult.deny(
        ...
        return AuthorizationResult.deny(
            ...
            denied_permissions=[
                Permission(
                    resource_type=resource_type, action=action, resource_id=resource_id
                )
            ],
        )
