...
class Permission(BaseModel):
    ...
    action: Action
    resource_id: Optional[str] = Field(None, description="Specific resource ID or wildcard")
    conditions: Dict[str, Any] = Field(default_factory=dict, description="Additional conditions")
    
    def matches(self, resource_type: ResourceType, action: Action, resource_id: Optional[str] = None) -> bool:
    ...
    def get_default_permissions(cls, role: Role) -> List[Permission]:
        ...
        if role == Role.OWNER:
            return [
                ...
                Permission(resource_type=ResourceType.NAMESPACE, action=Action.CREATE_REPO),
                Permission(resource_type=ResourceType.NAMESPACE, action=Action.DELETE_REPO),
                Permission(resource_type=ResourceType.NAMESPACE, action=Action.LIST_REPOS),
                Permission(resource_type=ResourceType.NAMESPACE, action=Action.MANAGE_USERS),
                ...
                # Token management
                Permission(resource_type=ResourceType.TOKEN, action=Action.CREATE_TOKEN),
                Permission(resource_type=ResourceType.TOKEN, action=Action.REVOKE_TOKEN),
...
class PermissionMatrix(BaseModel):
    ...
    def add_grant(self, grant: PermissionGrant) -> None:
        ...
        self.grants = [
            ...
            if not (g.grantee_id == grant.grantee_id and 
                   g.resource_type == grant.resource_type and
    ...
    def remove_grant(
        ...
        self.grants = [
            g for g in self.grants
            if not (g.grantee_id == grantee_id and 
                   g.resource_type == resource_type and
...
class AuthorizationResult(BaseModel):
    ...
    def deny(
        ...
        denied_permissions: List[Permission] = None
    ) -> "AuthorizationResult":
    ...
    def default_rules(cls) -> List["PermissionInheritance"]:
        ...
        return [
            ...
            cls(
                ...
                inherited_actions=[Action.READ, Action.WRITE, Action.ADMIN, Action.DELETE]
            ),
            cls(
                parent_resource_type=ResourceType.SYSTEM,
                child_resource_type=ResourceType.REPOSITORY,
