...
from src.plugins.base import (Plugin, PluginDependencyError, PluginError,
    ...
    def validate_plugin(self, plugin: Plugin) -> bool:
        ...
        if not isinstance(plugin, Plugin):
            raise PluginValidationError(
                f"Object is not a Plugin instance: {type(plugin)}"
            )
        ...
        for method_name in required_methods:
            ...
            if not callable(method):
                raise PluginValidationError(
                    f"Plugin method is not callable: {method_name}"
                )
