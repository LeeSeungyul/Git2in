...
class PluginResourceConfig(BaseModel):
    ...
    cpu_time: Optional[int] = Field(None, ge=1, le=3600, description="CPU time limit in seconds")
    memory: Optional[int] = Field(None, ge=1048576, le=2147483648, description="Memory limit in bytes")
    file_size: Optional[int] = Field(None, ge=0, le=1073741824, description="Max file size in bytes")
    file_descriptors: Optional[int] = Field(None, ge=10, le=1000, description="Max file descriptors")
    processes: Optional[int] = Field(None, ge=1, le=100, description="Max processes")
    timeout: Optional[float] = Field(30.0, ge=1.0, le=300.0, description="Execution timeout in seconds")
...
class PluginConfig(BaseModel):
    ...
    # Resource limits
    resources: Optional[PluginResourceConfig] = Field(None, description="Resource limits")
    ...
    def load_config_file(self, file_path: Union[str, Path]) -> Dict[str, PluginConfig]:
        ...
        try:
            ...
            if file_path.suffix in [".yml", ".yaml"]:
                ...
                raise PluginConfigurationError(f"Unsupported file format: {file_path.suffix}")
            
            # Parse plugins section
            if "plugins" not in data:
                raise PluginConfigurationError("Missing 'plugins' section in configuration")
            ...
            raise PluginConfigurationError(f"Failed to load configuration from {file_path}: {e}")
    
    def load_directory(self, directory: Optional[Path] = None) -> Dict[str, PluginConfig]:
        """
