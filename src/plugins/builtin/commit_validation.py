...
class CommitValidationPlugin(Plugin):
    ...
    def _compile_sensitive_patterns(self) -> List[tuple[Pattern, str]]:
        ...
        default_sensitive = [
            ...
            (
                r"(?i)(secret[_-]?key|secret)\s*[:=]\s*['\"]?[a-zA-Z0-9]{20,}",
                "Secret key",
            ),
    ...
    def _validate_files(self, context: PluginContext) -> List[str]:
        ...
        for file_path in context.operation.files_changed:
            ...
            file_size = context.operation.metadata.get("file_sizes", {}).get(
                file_path, 0
            )
        ...
        for file_path, content in file_contents.items():
            ...
            for pattern, description in self.sensitive_patterns:
                if pattern.search(content):
                    detections.append(
                        f"Sensitive content detected in {file_path}: {description}"
                    )
    ...
    async def post_receive(self, context: PluginContext) -> PluginResult:
        ...
        total_size = sum(context.operation.metadata.get("file_sizes", {}).values())
