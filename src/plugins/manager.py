...
from src.plugins.config import PluginConfigManager, PluginConfig
...
class PluginManager:
        ...
        try:
            if config and config.executable:
                ...
                plugin = ExternalPlugin(
                    ...
                    timeout=config.resources.timeout if config and config.resources else 30.0,
    ...
    async def _execute_sandboxed(
        ...
        limits = config.resources.to_resource_limits() if config.resources else ResourceLimits()
    ...
    def _update_circuit_breaker(self, plugin_name: str, result: PluginResult):
        ...
        if result.status in [PluginStatus.ERROR, PluginStatus.TIMEOUT]:
            ...
            if self._failure_counts[plugin_name] >= self.circuit_breaker_threshold:
                self._disabled_until[plugin_name] = time.time() + self.circuit_breaker_timeout
                logger.warning(
