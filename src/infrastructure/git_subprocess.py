...
class GitBackendProcess:
    ...
    def _create_cgi_environment(
        ...
        env.update({
            "GIT_PROJECT_ROOT": str(self.repository_path.parent.parent),  # Base repositories path
        ...
        if http_headers:
            for header, value in http_headers.items():
                ...
                if cgi_name not in ["HTTP_CONTENT_TYPE", "HTTP_CONTENT_LENGTH", "HTTP_AUTHORIZATION"]:
                    env[cgi_name] = value
            ...
            while True:
                ...
                try:
                    chunk = await asyncio.wait_for(
                        ...
                        timeout=min(10.0, self.timeout - (time.time() - self.start_time))
                    )
