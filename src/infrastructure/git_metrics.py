...
from src.infrastructure.metrics import (
    MetricsContext, git_negotiation_rounds,
    git_object_enumeration_duration_seconds, git_operation_duration_seconds,
    git_operations_total, git_pack_objects_bytes,
    git_ref_advertisement_duration_seconds, repository_size_bytes)
...
class GitMetricsTracker:
    ...
    def start_operation(
        self, operation_id: str, operation_type: str, namespace: str, repository: str
    ) -> None:
    ...
    def end_operation(
        ...
        if start_time:
            ...
            git_operation_duration_seconds.labels(
                operation=operation_type, namespace=namespace, repository=repository
            ).observe(duration)
    ...
    def track_ref_advertisement(
        self, namespace: str, repository: str, duration: float
    ) -> None:
    ...
    def track_object_enumeration(
        self, operation_type: str, namespace: str, repository: str, duration: float
    ) -> None:
    ...
    async def update_repository_size(
        self, namespace: str, repository: str, repo_path: Path
    ) -> None:
...
class GitOperationMetrics:
    ...
    def __enter__(self):
        git_metrics.start_operation(
            self.operation_id, self.operation_type, self.namespace, self.repository
        )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            git_metrics.track_ref_advertisement(
                self.namespace, self.repository, duration
            )
...
class ObjectEnumerationMetrics:
    ...
    def __init__(self, operation_type: str, namespace: str, repository: str):
        self.operation_type = operation_type
        self.namespace = namespace
        self.repository = repository
        self.start_time = None
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            git_metrics.track_object_enumeration(
                self.operation_type, self.namespace, self.repository, duration
            )
