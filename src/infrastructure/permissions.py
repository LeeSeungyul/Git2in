...
from src.core.exceptions import AuthorizationError, InternalServerError
...
class PermissionManager:
    ...
    def set_namespace_permissions(self, path: Path) -> None:
        ...
        try:
            ...
            logger.error("namespace_permissions_failed", path=str(path), error=str(e))
    ...
    def set_repository_permissions(self, path: Path) -> None:
        ...
        try:
            ...
            logger.error("repository_permissions_failed", path=str(path), error=str(e))
    ...
    def check_read_permission(self, path: Path) -> bool:
        ...
        try:
            ...
            logger.warning("permission_check_failed", path=str(path), error=str(e))
            return False
    ...
    def validate_namespace_access(
        self, namespace_path: Path, require_write: bool = False
    ) -> None:
        ...
        if not self.check_execute_permission(namespace_path):
            raise AuthorizationError(
                f"No execute access to namespace: {namespace_path}"
            )
    ...
    def validate_repository_access(
        self, repo_path: Path, require_write: bool = False
    ) -> None:
    ...
    def get_path_ownership(self, path: Path) -> Tuple[int, int, str, str]:
        ...
        try:
            ...
            logger.error("ownership_check_failed", path=str(path), error=str(e))
            raise InternalServerError(f"Failed to get ownership info: {str(e)}")

    def set_ownership(
        self, path: Path, uid: Optional[int] = None, gid: Optional[int] = None
    ) -> None:
        ...
        try:
            ...
            logger.debug("ownership_set", path=str(path), uid=uid, gid=gid)
            ...
            logger.error("ownership_set_failed", path=str(path), error=str(e))
    ...
    def enforce_namespace_isolation(
        ...
        try:
            ...
            logger.info("namespace_isolation_enforced", namespace=namespace_path.name)
    ...
    def get_permission_string(self, path: Path) -> str:
        ...
        try:
            ...
            logger.error("permission_string_failed", path=str(path), error=str(e))
            return "----------"
