...
class LockManager:
    ...
    @contextmanager
    def acquire_lock(
        self, resource_id: str, timeout: float = 30.0, exclusive: bool = True
    ):
        ...
        try:
            ...
            if not memory_lock.acquire(timeout=timeout):
                ...
                raise ConflictError(
                    f"Failed to acquire lock for {resource_id}: timeout"
                )
            ...
            while True:
                try:
                    ...
                    if time.time() >= deadline:
                        self._metrics["timeouts"] += 1
                        raise ConflictError(
                            f"Failed to acquire file lock for {resource_id}: timeout"
                        )
        ...
        with self._global_lock:
            return {
                **self._metrics,
                ...
                "total_contentions": sum(self._lock_counts.values())
                - len(self._lock_counts),
            }
    ...
    def cleanup_stale_locks(self, max_age_seconds: int = 3600) -> int:
        ...
        for lock_file in self.lock_dir.glob("*.lock"):
            try:
                ...
                logger.warning(
                    "stale_lock_cleanup_failed", file=str(lock_file), error=str(e)
                )
...
class OperationQueue:
    ...
    def enqueue(self, resource_id: str, operation: Callable, priority: int = 0) -> None:
    ...
    def dequeue(
        ...
        with self._lock:
            ...
            if queue:
                ...
                logger.debug(
                    "operation_dequeued", resource_id=resource_id, remaining=len(queue)
                )
...
class ConcurrentOperationManager:
    ...
    @contextmanager
    def execute_exclusive(self, resource_id: str, timeout: float = 30.0):
        """Execute an operation with exclusive access to a resource"""
        with self.lock_manager.acquire_lock(
            resource_id, timeout=timeout, exclusive=True
        ):
            yield

    @contextmanager
    def execute_shared(self, resource_id: str, timeout: float = 30.0):
        """Execute an operation with shared access to a resource"""
        with self.lock_manager.acquire_lock(
            resource_id, timeout=timeout, exclusive=False
        ):
            yield
    ...
    def execute_with_retry(
        ...
        return self.retry_manager.execute_with_retry(
            wrapped_operation, operation_name, should_retry
        )
