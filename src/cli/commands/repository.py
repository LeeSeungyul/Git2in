...
def create_repository(
    ...
    try:
        ...
        cli_ctx.formatter.print_error(
            "Invalid repository name format. Use: namespace/repo"
        )
        raise typer.Exit(1)
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            cli_ctx.formatter.print_success(
                f"Repository '{full_name}' created successfully"
            )
...
def list_repositories(
    ...
    filter: Optional[str] = typer.Option(
        None, "--filter", "-f", help="Filter repositories by name"
    ),
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            if filter:
                repositories = [
                    repo for repo in repositories if filter in repo.get("name", "")
                ]
...
def delete_repository(
    ...
    if not force:
        console.print(
            f"[bold red]Warning:[/bold red] This will permanently delete the repository and all its data!"
        )
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            cli_ctx.formatter.print_success(
                f"Repository '{full_name}' deleted successfully"
            )
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            result = subprocess.run(
                ["git", "clone", clone_url, target_dir], capture_output=True, text=True
            )
