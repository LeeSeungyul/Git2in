...
def select_scopes_interactive() -> List[str]:
    ...
    if not selected:
        console.print("[yellow]No scopes selected. Using default read-only scopes.[/yellow]")
...
def create_token(
    ...
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", help="Interactive scope selection"),
    scope: Optional[List[str]] = typer.Option(None, "--scope", "-s", help="Token scopes (can be used multiple times)"),
    expires_in: Optional[int] = typer.Option(None, "--expires-in", help="Token expiration in days"),
    output_file: Optional[str] = typer.Option(None, "--output", "-o", help="Save token to file"),
):
    """
    ...
    try:
        with cli_ctx.get_api_client() as client:
                ...
                if output_file:
                    try:
                        with open(output_file, "w") as f:
                            f.write(token_value)
                        console.print(f"[green]✓[/green] Token saved to '{output_file}'")
            ...
            console.print(f"  curl -H 'Authorization: Bearer {token_value[:8]}...' {cli_ctx.config.get('api_endpoint')}/api/v1/namespaces")
...
def list_tokens(
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            columns = ["id", "name", "scopes_display", "last_used_display", "expires_display", "created_at"]
...
def validate_token(
    ctx: typer.Context,
    token: Optional[str] = typer.Option(None, "--token", "-t", help="Token to validate (uses configured token if not specified)"),
):
    """
    ...
    try:
        ...
        if token:
            ...
            client = APIClient(base_url=api_endpoint, auth_token=token, debug=cli_ctx.debug)
        else:
...
def rotate_token(
    ...
    keep_scopes: bool = typer.Option(True, "--keep-scopes/--new-scopes", help="Keep existing scopes"),
    expires_in: Optional[int] = typer.Option(None, "--expires-in", help="New token expiration in days"),
):
    """
    ...
    try:
        with cli_ctx.get_api_client() as client:
            ...
            if keep_scopes:
                scopes = old_token.get("scopes", ["read:namespace", "read:repository", "read:user"])
            else:
            ...
            token_value = new_token_result.get("token", new_token_result.get("access_token"))
            if token_value:
            ...
            console.print(f"[green]✓[/green] New token '{new_token_result.get('id')}' has been created")
