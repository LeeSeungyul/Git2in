...
def create_user(
    ...
    full_name: Optional[str] = typer.Option(
        None, "--full-name", "-n", help="Full name"
    ),
    role: Optional[str] = typer.Option(
        "user", "--role", "-r", help="User role (admin, user)"
    ),
    ...
    try:
        ...
        if e.status_code == 409:
            cli_ctx.formatter.print_error(
                f"User '{username}' or email '{email}' already exists"
            )
        else:
...
def list_users(
    ...
    active: Optional[bool] = typer.Option(
        None, "--active/--inactive", help="Filter by status"
    ),
...
def update_user(
    ...
    email: Optional[str] = typer.Option(
        None, "--email", "-e", help="New email address"
    ),
    full_name: Optional[str] = typer.Option(
        None, "--full-name", "-n", help="New full name"
    ),
    role: Optional[str] = typer.Option(None, "--role", "-r", help="New role"),
    active: Optional[bool] = typer.Option(
        None, "--active/--inactive", help="Account status"
    ),
):
    """
...
def delete_user(
    ...
    if not force:
        if cascade:
            console.print(
                "[bold red]Warning:[/bold red] This will delete the user and ALL associated data!"
            )
...
def reset_password(
    ...
    generate: bool = typer.Option(
        True, "--generate/--prompt", help="Generate or prompt for password"
    ),
):
    """
    Reset user password.
    ...
    try:
        with cli_ctx.get_api_client() as client:
            if generate:
                ...
                result = client.post(
                    f"/api/v1/users/{username}/reset-password", json_data={}
                )
                ...
                if "password" in result:
                    ...
                    cli_ctx.formatter.print_success(
