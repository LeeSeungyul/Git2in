...
from src.api.auth.dependencies import (AdminToken, CurrentToken,
                                       get_current_token, require_admin,
                                       require_token_create,
                                       require_token_revoke)
from src.core.auth.models import TokenRequest, TokenScope, TokenType
...
async def create_token(request: Request, token_request: TokenCreateRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid scope: {str(e)}"
        )
...
async def refresh_token(request: Request, refresh_request: RefreshTokenRequest):
    ...
    logger.info(
        "token_refreshed", user_id=claims.sub, correlation_id=get_correlation_id()
    )
...
@router.post("/revoke")
async def revoke_token(revoke_request: TokenRevokeRequest, current_token: CurrentToken):
    """Revoke a token"""
...
async def introspect_token(
    request: TokenIntrospectRequest, current_token: Optional[CurrentToken] = None
):
...
@router.get("/sessions")
async def list_sessions(current_token: CurrentToken):
...
async def terminate_all_sessions(current_token: CurrentToken):
    ...
    logger.info("all_sessions_terminated", user_id=current_token.sub, count=count)
...
async def create_api_key(
    ...
    api_key = token_service.create_api_key(
        user_id=current_token.user_id or UUID(current_token.sub),
