...
from src.infrastructure.git_protocol import (
    GitService, GitContentType, GitHeaders, 
    GitProtocolValidator, PktLineParser
)
...
from src.infrastructure.hooks import hook_manager, HookEvent, HookContext
from src.api.auth.dependencies import OptionalToken, require_repo_read, require_repo_write
...
from src.infrastructure.audit.logger import audit_logger, AuditAction, AuditResult
...
async def get_info_refs(
    ...
    is_smart = git_service is not None and GitHeaders.is_smart_request(dict(request.headers))
...
async def post_upload_pack(
    ...
    if not GitHeaders.validate_content_type(
        ...
        logger.warning(
            "git_upload_pack_bad_content_type",
    ...
    if hook_context.has_errors():
        logger.warning(
            "git_upload_pack_blocked_by_hook",
            errors=hook_context.errors
        )
    ...
    async def stream_response():
        ...
        async with backend.execute(
            ...
            try:
                ...
                headers, body_stream = await GitBackendStreamProcessor.parse_cgi_headers(output_stream)
...
async def post_receive_pack(
    ...
    if not GitHeaders.validate_content_type(
        content_type,
        GitContentType.RECEIVE_PACK_REQUEST
    ):
        logger.warning(
            "git_receive_pack_bad_content_type",
            content_type=content_type
        )
