...
class MetricsAccessControl:
    ...
    def __init__(self):
        ...
        self.allowed_ips = settings.metrics_allowed_ips if hasattr(settings, 'metrics_allowed_ips') else [
            "127.0.0.1",
            "::1"  # IPv6 localhost
        ]
    ...
    def check_access(
        self,
        request: Request,
        token: Optional[OptionalToken] = None
    ) -> bool:
        """Check if request has access to metrics"""
        
        # Check if metrics endpoint is enabled
        if not settings.metrics_enabled if hasattr(settings, 'metrics_enabled') else True:
            return False
        ...
        if token and hasattr(token, 'has_scope'):
            if token.has_scope(self.required_scope):
                logger.debug(
                    "metrics_access_granted_by_token",
                    user_id=token.sub
                )
                return True
        ...
        if settings.is_production:
            try:
                ...
                logger.error(
                    "metrics_ip_filter_check_failed",
                    error=str(e)
                )
...
async def get_metrics_json(
    ...
    try:
        ...
        logger.error(
            "metrics_json_generation_failed",
            error=str(e),
            exc_info=True
        )
        raise HTTPException(
