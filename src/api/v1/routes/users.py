...
from src.api.v1.models.pagination import (PaginatedResponse, PaginationParams,
                                          ...
                                          get_sort_params)
from src.api.v1.models.user import (PasswordChangeRequest, UserCreateRequest,
                                    UserFilterParams, UserPublicResponse,
...
async def list_users(
    ...
    try:
        ...
        logger.error("user_list_failed", error=str(e), correlation_id=correlation_id)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to list users", correlation_id=correlation_id
            ).model_dump(),
        )
...
async def create_user(request: UserCreateRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to create user", correlation_id=correlation_id
            ).model_dump(),
        )
...
async def get_current_user(current_token: TokenClaims = Depends(get_current_token)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to get user profile", correlation_id=correlation_id
            ).model_dump(),
        )
...
async def get_user(
    user_id: UUID, current_token: Optional[TokenClaims] = Depends(get_current_token)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to get user", correlation_id=correlation_id
            ).model_dump(),
        )
...
async def update_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to update user", correlation_id=correlation_id
            ).model_dump(),
        )
...
async def delete_user(
    user_id: UUID, current_token: TokenClaims = Depends(get_current_token)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse.internal_error(
                message="Failed to delete user", correlation_id=correlation_id
            ).model_dump(),
        )
