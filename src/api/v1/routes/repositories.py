...
from fastapi import APIRouter, Depends, HTTPException, Query, Response, Request, status, BackgroundTasks

from src.api.v1.models.repository import (
    RepositoryCreateRequest,
    RepositoryUpdateRequest,
    RepositoryResponse,
    RepositoryFilterParams,
    RepositoryStatsResponse,
    RepositoryCollaboratorRequest,
...
async def create_repository(
    ...
    try:
        ...
        existing = await repository_service.get_repository_by_name(namespace_obj.id, request.name)
        if existing:
            raise HTTPException(
...
async def get_repository(
    ...
    try:
        ...
        # Get repository
        repository = await repository_service.get_repository_by_name(namespace_obj.id, repo)
        if not repository:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=ErrorResponse.not_found(
