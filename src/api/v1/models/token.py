...
class TokenCreateRequest(BaseModel):
    ...
    expires_in_days: int = Field(
        30, ge=1, le=365, description="Token expiration in days"
    )
...
class TokenResponse(BaseModel):
    ...
    token: Optional[str] = Field(
        None, description="Token value (only shown on creation)"
    )
...
class TokenRotateRequest(BaseModel):
    ...
    expires_in_days: int = Field(
        30, ge=1, le=365, description="New token expiration in days"
    )
...
class TokenStatistics(BaseModel):
    ...
    tokens_by_scope: Dict[str, int] = Field(
        default_factory=dict, description="Token count by scope"
    )
    recent_activity: List[Dict[str, Any]] = Field(
        default_factory=list, description="Recent token activity"
    )
