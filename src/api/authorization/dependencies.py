...
from src.core.authorization.models import (Action, AuthorizationResult,
                                           ResourceType)
...
class RequirePermission:
    ...
    async def __call__(
        self, request: Request, token_claims: TokenClaims = Depends(get_current_token)
    ) -> AuthorizationResult:
    ...
    async def __call__(
        ...
        if action in [Action.VIEW_USER, Action.EDIT_USER]:
            if token_claims.sub == user_id or str(token_claims.user_id) == user_id:
                return AuthorizationResult.allow(reason="User can manage own profile")
...
async def get_authorization_context(
    request: Request, token_claims: TokenClaims = Depends(get_current_token)
