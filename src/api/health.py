...
async def readiness_check() -> Dict[str, Any]:
    ...
    checks["filesystem"] = {"healthy": fs_healthy, "message": fs_message}
    if not fs_healthy:
        all_healthy = False

    # Check git binary
    git_healthy, git_message = health_checker.check_git_binary()
    checks["git_binary"] = {"healthy": git_healthy, "message": git_message}
    if not git_healthy:
        all_healthy = False
...
async def detailed_health_check() -> Dict[str, Any]:
    ...
    # Disk space check
    disk_healthy, disk_message = health_checker.check_disk_space()
    checks["disk_space"] = {"healthy": disk_healthy, "message": disk_message}

    # Memory check
    mem_healthy, mem_message = health_checker.check_memory()
    checks["memory"] = {"healthy": mem_healthy, "message": mem_message}
    ...
    response = {
        "status": (
            "healthy"
            if all(
                c.get("healthy", False) for c in checks.values() if not c.get("warning")
            )
            else "degraded"
        ),
        "timestamp": datetime.utcnow().isoformat(),
